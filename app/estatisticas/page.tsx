...
import { EstatisticasList } from "@/components/EstatisticasList";
...
export default async function EstatisticasPage({
  ...
  const [teamGames, teamAthletes] = await Promise.all([
        ...
        <div className="max-w-275 mx-auto">
          <div className="mb-6">
            <div className="font-heading font-semibold text-[13px] tracking-[0.24em] text-brand-red uppercase">
              Estatísticas · {findTeamLabel(allTeams, teamId)}
            </div>
          ...
          {teamGames.length === 0 ? (
            <div className="border border-dashed border-border-dash rounded-xl py-16 text-center text-muted-2">
              Nenhum jogo cadastrado ainda. Crie um jogo em &ldquo;Jogos&rdquo; primeiro.
            </div>
          ) : (
            <EstatisticasList games={teamGames} teamAthletes={teamAthletes} />
          )}
        </div>
      </main>
    </div>
